...
void DisplayPlayersMap (char map[10][10], float ShipsPotions[5][2], float ShipSize[5][2])
{
    ...
    int xchange = ceil(gScreenRect.w/56.875);
    int ychange = ceil(gScreenRect.h/30.625);
    enum xy {x=0,y};
    int counter=0;
    int sizes [5]= {5,2,5,5,2};
...
//std::cout<<gScreenRect.w/20+IntroW/5<<std::endl;
                ...
                if (Menu==true)
                {
                    ...
                    gSplashTexture.render(( gScreenRect.w - gScreenRect.w/2 - gScreenRect.w/3 - gScreenRect.w/5) / 2 +30,0,menu,IntroW, IntroH);
                    //Update screen
                    SDL_RenderPresent( gRenderer );
                    //Menu=true;
                ...
                else if (StartGame==false and Menu==false)
                {
                    ...
                    //SDL_SetRenderDrawColor( gRenderer, 0, 0, 0, 0 );
                    //SDL_RenderClear( gRenderer );
                    gSplashTexture.render(( gScreenRect.w - gScreenRect.w/2 - gScreenRect.w/3 - gScreenRect.w/5) / 2 +30,0,battleshipBoard,IntroW, IntroH);
                    int temp=10;
                    for (int i=0; i<5; i++)
                    {
                    ...
                    if (trackingGeneral==false)
                    {
                        if (OnGrid(ShipSize,ShipsPotions))
                        {
                            ...
                            if (Clickx>gScreenRect.w/1.27 and Clickx<gScreenRect.w/1.27+gScreenRect.w/5 and Clicky>gScreenRect.h/1.7 and Clicky<gScreenRect.h/1.7+gScreenRect.h/10)
                            {
                                ...
                                LockShips=true;
                                StartGame=true;
                                PlayersMapSetter(ShipSize,ShipsPotions,map);
                                DisplayPlayersMap(map,ShipsPotions,ShipSize);
                            }
                        }
                ...
                else if (StartGame==true )
                {
                    ...
                    gSplashTexture.render(1450,535,MiniMap,350, 350);
